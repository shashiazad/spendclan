...
### 1.1 Mobile Dashboard User Interface Spec
The frontend layout conforms to a premium Obsidian Glass theme with violet accents, optimized for touch-based interactions on mobile devices:

![SpendClan Mobile Dashboard UI](public/spendclan_mobile_dashboard.png)

---

### 1.2 Architecture Tier Design
...
### 1.3 Core Data Flows & Workflows

#### 1.3.1 Group Expense Splitting & Cache Invalidation Pipeline
...
sequenceDiagram
    ...
    participant API as Next.js API Route (/api/groups/id/expenses)
    participant DB as PostgreSQL (Prisma)
    participant Cache as Next.js Cache (revalidateTag)

    User->>API: POST /api/groups/[id]/expenses (Amount, Split Type, Splits)
    API->>DB: Start transaction
    Note over DB: Create GroupExpense & GroupSplit records
    DB-->>API: Success
    API->>Cache: Invalidate tags for all members (dashboard-[userId])
    API-->>User: 200 OK (Updated Balances)
```

#### 1.3.2 AI Advisor Context Compilation Flow
Ensures the LLM receives structured, deterministic contextual financial snapshots:

```mermaid
sequenceDiagram
    autonumber
    actor User as User Page
    participant API as Next.js API (/api/ai/insights)
    participant DB as PostgreSQL
    participant AI as Gemini API

    User->>API: GET /api/ai/insights
    alt Cache hit
        API-->>User: Return Cached Analysis
    else Cache miss
        API->>DB: Query last 3 months of expenses, income, recurring logs
        DB-->>API: Financial Raw Data
        API->>API: Compile AIFinancialContext & prompt
        API->>AI: generateContent(prompt)
        AI-->>API: JSON String Response
        API->>API: Cache parsed JSON
        API-->>User: Return Insights (Spending Score, Tips)
    end
```
...
### 2.1 Database Schema (Prisma Specifications)

The database schema utilizes direct relational fields, compound indices, and strict deletion rules to maintain referential integrity.

```mermaid
erDiagram
    User ||--o{ PersonalExpense : tracks
    User ||--o{ Income : earns
    User ||--o{ Saving : saves
    ...
    User ||--o{ GroupSplit : owes
    User ||--o{ Settlement : transfers
    Group ||--o{ GroupMember : has
    Group ||--o{ GroupExpense : contains
    Group ||--o{ Settlement : records
    GroupExpense ||--o{ GroupSplit : divides
```
...
3. **Password Security**: Credentials passwords are hashed using `bcryptjs` with a factor of 12. Security answers are normalized (lowercased/trimmed) and compared after hashing to prevent timing attacks.
